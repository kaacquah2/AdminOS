-- ============================================
-- AdminOS - Seed PMO (Project Management Office) Data
-- ============================================
-- This script populates sample data for PMO modules
-- 
-- Tables covered:
-- - pmo_projects
-- - pmo_milestones
-- - pmo_project_resources
-- - pmo_project_risks
-- - pmo_project_issues
...
-- ============================================
-- 1. CREATE BASE PROJECTS (if they don't exist)
-- ============================================

-- Insert base projects that PMO projects will reference
-- Using DO block to ensure all projects are created
DO $$
DECLARE
...
-- Get PMO manager (or use first available user)
DO $$
DECLARE
  v_pm_id UUID;
...
BEGIN
  ...
  AND (role = 'dept_manager' OR full_name LIKE '%Young%' OR full_name LIKE '%Taylor%' OR full_name LIKE '%Reed%')
  ORDER BY CASE 
    WHEN full_name LIKE '%Young%' THEN 1
    WHEN full_name LIKE '%Taylor%' THEN 2
    WHEN full_name LIKE '%Reed%' THEN 3
    ELSE 4
  END
  LIMIT 1;
  
  -- If no PMO manager found, get any dept_manager
  ...
  -- Get creator
  SELECT id, full_name INTO v_creator_id, v_creator_name
  FROM public.user_profiles 
  WHERE department = 'Project Management' AND role = 'dept_manager'
  LIMIT 1;
  
  -- If no creator found, use PM
...
-- ============================================
-- 3. PMO MILESTONES (Simplified - using direct inserts)
-- ============================================
...
BEGIN
  -- Get assignee
  SELECT id, full_name INTO v_assignee_id, v_assignee_name
  FROM public.user_profiles 
  WHERE department = 'Project Management'
  LIMIT 1;
  
  -- If no PMO user, get any user
  ...
  -- ERP System Implementation milestones
  SELECT pp.id INTO v_pmo_project_id
  FROM public.pmo_projects pp
  INNER JOIN public.projects p ON pp.project_id = p.id
  WHERE p.name = 'ERP System Implementation'
  LIMIT 1;
  
  IF v_pmo_project_id IS NOT NULL THEN
    INSERT INTO public.pmo_milestones (milestone_number, pmo_project_id, milestone_name, milestone_type, description, planned_date, status, completion_percentage, is_critical, assigned_to, assigned_to_name)
    VALUES
      ('MS-' || EXTRACT(YEAR FROM CURRENT_DATE) || '-0001', v_pmo_project_id, 'Requirements Gathering Complete', 'phase_gate', 'All requirements documented and approved', CURRENT_DATE - INTERVAL '30 days', 'completed', 100, true, v_assignee_id, v_assignee_name),
      ('MS-' || EXTRACT(YEAR FROM CURRENT_DATE) || '-0002', v_pmo_project_id, 'System Configuration Complete', 'phase_gate', 'ERP system configured per requirements', CURRENT_DATE + INTERVAL '15 days', 'in_progress', 60, true, v_assignee_id, v_assignee_name),
      ('MS-' || EXTRACT(YEAR FROM CURRENT_DATE) || '-0003', v_pmo_project_id, 'User Acceptance Testing', 'phase_gate', 'UAT completed successfully', CURRENT_DATE + INTERVAL '45 days', 'pending', 0, true, v_assignee_id, v_assignee_name),
      ('MS-' || EXTRACT(YEAR FROM CURRENT_DATE) || '-0004', v_pmo_project_id, 'Go-Live', 'go_live', 'System goes live for all users', CURRENT_DATE + INTERVAL '90 days', 'pending', 0, true, v_assignee_id, v_assignee_name)
  ...
  FROM public.pmo_projects pp
  INNER JOIN public.projects p ON pp.project_id = p.id
  WHERE p.name = 'Digital Transformation'
  LIMIT 1;
  
  IF v_pmo_project_id IS NOT NULL THEN
    INSERT INTO public.pmo_milestones (milestone_number, pmo_project_id, milestone_name, milestone_type, description, planned_date, status, completion_percentage, is_critical, assigned_to, assigned_to_name)
    VALUES
      ('MS-' || EXTRACT(YEAR FROM CURRENT_DATE) || '-0005', v_pmo_project_id, 'Strategy Approved', 'approval', 'Digital transformation strategy approved', CURRENT_DATE - INTERVAL '80 days', 'completed', 100, true, v_assignee_id, v_assignee_name),
      ('MS-' || EXTRACT(YEAR FROM CURRENT_DATE) || '-0006', v_pmo_project_id, 'Platform Selected', 'phase_gate', 'Technology platform selected', CURRENT_DATE - INTERVAL '50 days', 'completed', 100, true, v_assignee_id, v_assignee_name),
      ('MS-' || EXTRACT(YEAR FROM CURRENT_DATE) || '-0007', v_pmo_project_id, 'Development Phase 1', 'deliverable', 'First phase of development complete', CURRENT_DATE + INTERVAL '30 days', 'in_progress', 40, true, v_assignee_id, v_assignee_name),
      ('MS-' || EXTRACT(YEAR FROM CURRENT_DATE) || '-0008', v_pmo_project_id, 'Pilot Launch', 'go_live', 'Pilot launch with select customers', CURRENT_DATE + INTERVAL '120 days', 'pending', 0, true, v_assignee_id, v_assignee_name),
      ('MS-' || EXTRACT(YEAR FROM CURRENT_DATE) || '-0009', v_pmo_project_id, 'Full Launch', 'go_live', 'Full digital transformation launch', CURRENT_DATE + INTERVAL '180 days', 'pending', 0, true, v_assignee_id, v_assignee_name)
    ON CONFLICT (milestone_number) DO NOTHING;
  END IF;
END $$;
...
DECLARE
  ...
  v_identifier_name VARCHAR(255);
BEGIN
  -- Get identifier
  SELECT id, full_name INTO v_identifier_id, v_identifier_name
  FROM public.user_profiles 
  WHERE department = 'Project Management'
  LIMIT 1;
  ...
  IF v_pmo_project_id IS NOT NULL THEN
    INSERT INTO public.pmo_project_risks (risk_number, pmo_project_id, risk_title, risk_description, risk_category, probability, impact, status, mitigation_strategy, identified_by, identified_by_name)
    VALUES
      ('RISK-' || EXTRACT(YEAR FROM CURRENT_DATE) || '-0001', v_pmo_project_id, 'User Resistance to Change', 'Employees may resist adopting new ERP system', 'resource', 'medium', 'high', 'open', 'Comprehensive training program, change management communication, user champions', v_identifier_id, v_identifier_name),
      ('RISK-' || EXTRACT(YEAR FROM CURRENT_DATE) || '-0002', v_pmo_project_id, 'Integration Complexity', 'Complex integration with existing systems may cause delays', 'technical', 'high', 'high', 'open', 'Early integration testing, dedicated integration team, phased rollout', v_identifier_id, v_identifier_name),
      ('RISK-' || EXTRACT(YEAR FROM CURRENT_DATE) || '-0003', v_pmo_project_id, 'Budget Overrun', 'Project costs may exceed allocated budget', 'financial', 'medium', 'medium', 'open', 'Regular budget reviews, cost tracking, contingency planning', v_identifier_id, v_identifier_name)
    ON CONFLICT (risk_number) DO NOTHING;
  END IF;
  ...
  IF v_pmo_project_id IS NOT NULL THEN
    INSERT INTO public.pmo_project_risks (risk_number, pmo_project_id, risk_title, risk_description, risk_category, probability, impact, status, mitigation_strategy, identified_by, identified_by_name)
    VALUES
      ('RISK-' || EXTRACT(YEAR FROM CURRENT_DATE) || '-0004', v_pmo_project_id, 'Technology Selection Risk', 'Selected technology may not meet requirements', 'technical', 'medium', 'critical', 'open', 'Proof of concept, vendor evaluation, technical assessment', v_identifier_id, v_identifier_name),
      ('RISK-' || EXTRACT(YEAR FROM CURRENT_DATE) || '-0005', v_pmo_project_id, 'Scope Creep', 'Project scope may expand beyond original plan', 'scope', 'high', 'high', 'mitigated', 'Change control process, regular scope reviews, stakeholder alignment', v_identifier_id, v_identifier_name)
    ON CONFLICT (risk_number) DO NOTHING;
  END IF;
END $$;

-- ============================================
-- 5. PMO PROJECT ISSUES
-- ============================================
...
DECLARE
  v_pmo_project_id UUID;
  v_reporter_id UUID;
  v_reporter_name VARCHAR(255);
BEGIN
  -- Get reporter
  SELECT id, full_name INTO v_reporter_id, v_reporter_name
  FROM public.user_profiles 
  WHERE department = 'Project Management'
  LIMIT 1;
  ...
  IF v_pmo_project_id IS NOT NULL THEN
    INSERT INTO public.pmo_project_issues (issue_number, pmo_project_id, issue_title, issue_description, issue_category, priority, status, impact_description, resolution_plan, reported_by, reported_by_name)
    VALUES
      ('ISS-' || EXTRACT(YEAR FROM CURRENT_DATE) || '-0001', v_pmo_project_id, 'Data Migration Delays', 'Data migration taking longer than expected', 'technical', 'high', 'open', 'May delay go-live date by 2 weeks', 'Additional resources allocated, extended timeline', v_reporter_id, v_reporter_name),
      ('ISS-' || EXTRACT(YEAR FROM CURRENT_DATE) || '-0002', v_pmo_project_id, 'Training Material Not Ready', 'Training materials not completed on schedule', 'quality', 'medium', 'in_progress', 'Training sessions may need to be rescheduled', 'Accelerated content development, revised training schedule', v_reporter_id, v_reporter_name)
    ON CONFLICT (issue_number) DO NOTHING;
  END IF;
END $$;

-- ============================================
-- 6. PMO PROJECT STAKEHOLDERS
-- ============================================
...
SELECT 
  pp.id as pmo_project_id,
  s.id as stakeholder_id,
  s.full_name as stakeholder_name,
  s.email as stakeholder_email,
  'sponsor' as stakeholder_role,
  'high' as interest_level,
  'high' as influence_level,
  'weekly' as communication_frequency
FROM public.pmo_projects pp
CROSS JOIN (
...
BEGIN
  ...
  -- ERP System Implementation status report
  SELECT pp.id INTO v_pmo_project_id
  FROM public.pmo_projects pp
  INNER JOIN public.projects p ON pp.project_id = p.id
  WHERE p.name = 'ERP System Implementation'
  ...
  IF v_pmo_project_id IS NOT NULL THEN
    INSERT INTO public.pmo_status_reports (
      report_number, pmo_project_id, report_period_start, report_period_end,
      report_type, status_summary, accomplishments, challenges, next_steps,
      overall_health, reported_by, reported_by_name
    )
    VALUES (
      'RPT-' || EXTRACT(YEAR FROM CURRENT_DATE) || '-0001',
      v_pmo_project_id,
      CURRENT_DATE - INTERVAL '7 days',
      CURRENT_DATE,
      'weekly',
      'Project is progressing well. System configuration is 60% complete. User training materials are being developed.',
      ARRAY['Requirements gathering completed', 'System configuration started', 'Training plan approved']::TEXT[],
      ARRAY['Data migration taking longer than expected', 'Some user resistance to change']::TEXT[],
      ARRAY['Complete system configuration', 'Begin user acceptance testing', 'Finalize training materials']::TEXT[],
      'yellow',
      v_reporter_id,
      v_reporter_name
    )
